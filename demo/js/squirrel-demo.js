...
(function () {
    ...
    function creatLoadMore(tabId) {
        var musicList = new SQ.LoadMore({
            API: "data/music-list-json.json?" + tabId,
    ...
    var tabs = new SQ.Tabs({
        ...
        API_URL: ["data/music-list-json.json", "data/music-list-json.json", "data/music-list-json.json"],
        ...
        show: function ($tabs, $panels, tabIndex) {
            //console.log("show")
            //$panels.eq(tabIndex).find(".J_ajaxWrap").empty();
            //$panels.addClass("J_loadMore" + tabIndex);
        },
        loaded: function (data, $activePanels) {
            //console.log("loaded")
            var tabIndex = $activePanels.index();
            var html = window.template.render("music-list", data);
            $activePanels.find(".J_ajaxWrap").addClass("J_loadMore" + tabIndex).append(html);
            if (imglazyload) {
                imglazyload.refresh();
            }
            creatLoadMore(".J_loadMore" + tabIndex);
        }
    });

    /*var musicList = new SQ.LoadMore({
        API: "data/music-list-json.json",
        EVE_EVENT_TYPE: "scroll",
        DOM_TRIGGER_TARGET: window,
        DOM_AJAX_WRAP: ".J_loadMore",
        CSS_STATE_BAR: ".music-list-loadmore",
        NUM_SCROLL_MAX_PAGE: 2,
        DATA_TYPE: "html",
        loaded: function (data, $ajaxWrap) {
    ...
    var popupDetail = new SQ.Popup({
        ...
        beforeShow: function () {
            var me = this;
            me.$popupContent.append($("#detail-template").html());
            return true;
        },
        show: function () {
            var me = this;
